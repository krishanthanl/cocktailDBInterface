...
describe('SearchBar Component', () => {
    let setSearchResults: jest.Mock<unknown[], unknown[], unknown>,
        setLoading: jest.Mock<unknown[], unknown[], unknown>
    ...
    it('calls searchCocktails on search button click', async () => {
        ...
        expect(setSearchResults).toHaveBeenCalledWith({
            searchResults: [{ id: 1, name: 'Margarita' }],
    ...
    it('calls fetchUniqueRandomCocktails on refresh button click', async () => {
        ...
        // render(<SearchBar setSearchResults={setSearchResults} setLoading={setLoading} />);
        ;(fetchUniqueRandomCocktails as jest.Mock).mockReturnValue(
            Promise.resolve([{ id: 2, name: 'Mojito' }]),
        )
        render(<SearchBar setSearchResults={setSearchResults} setLoading={setLoading} />)
        ...
        expect(setLoading).toHaveBeenCalledTimes(2) // true, then false
        expect(setSearchResults).toHaveBeenCalledWith({
            searchResults: [{ id: 2, name: 'Mojito' }],
